...
{
  struct list_elem *iterate = list_begin (&all_list);
  struct list_elem *end = list_end (&all_list);

  while(iterate != end) {

    struct thread *cur = list_entry (iterate, struct thread, allelem);
    if (cur->tid == tid) {
      return cur;
    }
               ...
               thread_func *function, void *aux) 
{
  ...
  /* Make sure the content of the stack cannot be accessed */
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  * Added content for Project 2 */
  ...
  /* Added content for Poject 3 */
  memset(t->file_list_array, 0, 128);
  t->directory = 1;
}
